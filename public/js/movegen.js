...
function GenerateMoves() {
	...
	if(GameBoard.side == COLOURS.WHITE) {
			...
			if( GameBoard.pieces[sq + 10] == PIECES.EMPTY ) {
				...
				if( (RanksBrd[sq] == RANKS.RANK_2) && (GameBoard.pieces[sq + 20] == PIECES.EMPTY) ) {				
					AddQuietMove( MOVE(sq, sq + 20, PIECES.EMPTY, PIECES.EMPTY, MFLAGPS ));
																										...
																										//sMove ile capture alanına düşen King'in capture edilmemesi için 
			if(SQOFFBOARD(sq + 9) == BOOL.FALSE && PieceCol[GameBoard.pieces[sq+9]] == COLOURS.BLACK && GameBoard.pieces[sq+9] != PIECES.bK) {
				AddWhitePawnCaptureMove(sq, sq + 9, GameBoard.pieces[sq+9]);
			}
			
			if(SQOFFBOARD(sq + 11) == BOOL.FALSE && PieceCol[GameBoard.pieces[sq+11]] == COLOURS.BLACK && GameBoard.pieces[sq+11] != PIECES.bK) {
				AddWhitePawnCaptureMove(sq, sq + 11, GameBoard.pieces[sq+11]);
			}			
			
			if(GameBoard.enPas[COLOURS.BLACK] != SQUARES.NO_SQ) {
				if(sq + 9 == GameBoard.enPas[1]) {
					AddEnPassantMove( MOVE(sq, sq+9, PIECES.EMPTY, PIECES.EMPTY, MFLAGEP ) );
				}
				
				if(sq + 11 == GameBoard.enPas[1]) {
		...
		for(pceNum = 0; pceNum < GameBoard.pceNum[pceType]; ++pceNum) {
			...
			if(SQOFFBOARD(sq - 9) == BOOL.FALSE && PieceCol[GameBoard.pieces[sq-9]] == COLOURS.WHITE && GameBoard.pieces[sq-9] != PIECES.wK) {
				AddBlackPawnCaptureMove(sq, sq - 9, GameBoard.pieces[sq-9]);
			}
			
			if(SQOFFBOARD(sq - 11) == BOOL.FALSE && PieceCol[GameBoard.pieces[sq-11]] == COLOURS.WHITE && GameBoard.pieces[sq-11] != PIECES.wK) {
				AddBlackPawnCaptureMove(sq, sq - 11, GameBoard.pieces[sq-11]);
			}			
			
			if(GameBoard.enPas[COLOURS.WHITE] != SQUARES.NO_SQ) {
				if(sq - 9 == GameBoard.enPas[0]) {
					AddEnPassantMove( MOVE(sq, sq-9, PIECES.EMPTY, PIECES.EMPTY, MFLAGEP ) );
				}
				
				if(sq - 11 == GameBoard.enPas[0]) {
					AddEnPassantMove( MOVE(sq, sq-11, PIECES.EMPTY, PIECES.EMPTY, MFLAGEP ) );
				}
			}
		}
	...
	while (pce != 0) {
		for(pceNum = 0; pceNum < GameBoard.pceNum[pce]; ++pceNum) {
			...
			for(index = 0; index < DirNum[pce]; index++) {
				...
				if(GameBoard.pieces[t_sq] == Kings[PieceCol[pce]^1]) continue;//sMove ile karşı tarafın King'inin yenmesi önleniyor.
				
				//King IsAtacked Sq'lara hareket edemeyeceğinden dolayı ilgili move'ın üretilmesine gerek yok.
				//King'lerin yan yana gelmesi durumundan sonra o pozisyondan kurutulma move'unun var olup olmadığının tespiti için buraya eklenmiştir.
				var IsAttacked = SqAttacked(t_sq, GameBoard.side^1);
				if(  pce == Kings[PieceCol[pce]] && IsAttacked == BOOL.TRUE ){
					continue;
				}
		...
		for(pceNum = 0; pceNum < GameBoard.pceNum[pce]; ++pceNum) {
			...
			for(index = 0; index < DirNum[pce]; index++) {
				...
				while( SQOFFBOARD(t_sq) == BOOL.FALSE ) {
					if(GameBoard.pieces[t_sq] == Kings[PieceCol[pce]^1]) break;//sMove ile capture alanına düşen King'in capture edilmemesi için															  
					if(GameBoard.pieces[t_sq] != PIECES.EMPTY ) {
						if(PieceCol[GameBoard.pieces[t_sq]] != GameBoard.side) {
							AddCaptureMove( MOVE(sq, t_sq, GameBoard.pieces[t_sq], PIECES.EMPTY, 0 ));
						}
