...
/*function MakeMove(move) {
	...
	if(side == GameController.Player) GameBoard.history[GameBoard.hisPly].uMove = move;
	if(side == GameController.Engine) GameBoard.history[GameBoard.hisPly].eMove = move;
    GameBoard.history[GameBoard.hisPly].fiftyMove = GameBoard.fiftyMove;
    GameBoard.history[GameBoard.hisPly].uEnPas = GameBoard.enPas[GameController.Player];
	GameBoard.history[GameBoard.hisPly].eEnPas = GameBoard.enPas[GameController.Engine];
    GameBoard.history[GameBoard.hisPly].castlePerm = GameBoard.castlePerm;
	...
	if(PiecePawn[GameBoard.pieces[from]] == BOOL.TRUE) {
        ...
        if( (move & MFLAGPS) != 0) {
            if(side==COLOURS.WHITE) {
				if(GameController.Player == COLOURS.WHITE){
					GameBoard.enPas[GameController.Player] = from+10;
				}else{
					GameBoard.enPas[GameController.Engine] = from+10;
				}            
			} else {
               if(GameController.Player == COLOURS.BLACK){
					GameBoard.enPas[GameController.Player] = from-10;
				}else{
					GameBoard.enPas[GameController.Engine] = from-10;
				}   
