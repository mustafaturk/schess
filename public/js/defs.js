...
var START_FEN = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR b KQkq - -"; // oyun başlar başlamaz satranç motorunun kullanıcı ile aynı anda düşünmeye başlaması için side w'dan b'ye alınmıştır. 
...
function HASH_EP() { 
	if(GameBoard.enPas[COLOURS.WHITE] != SQUARES.NO_SQ)	GameBoard.posKey ^= PieceKeys[GameBoard.enPas[COLOURS.WHITE]];
	if(GameBoard.enPas[COLOURS.BLACK] != SQUARES.NO_SQ)	GameBoard.posKey ^= PieceKeys[GameBoard.enPas[COLOURS.BLACK]];
	}

var GameController = {};
GameController.EngineSide = COLOURS.BLACK; //
GameController.PlayerSide = COLOURS.WHITE; // Şimdilik WHITE olarak belirlendi
...
var UserMove2 = {};
UserMove2.from = SQUARES.NO_SQ;
UserMove2.to = SQUARES.NO_SQ;

var MoveFound = [0,0];//new Array(2);

//to display history
var cMoves ="";
var hMoves ="";

var SideString= ['Player', 'Computer'];

var SideColourString = ['Beyaz', 'Siyah'];

var twoPlayerInfo;

var player = 0; // no player a init
var sideInfoFromServer = 2;//no side as init
var side = '';//no side
var mode = 1;//Öğrenme veya oyun modu

var uMv = NOMOVE; //userMove/yourMove kaydediliyor.
var oppMove = NOMOVE;//rakibin move kaydediliyor.

var socket = io.connect();
var moveDone = BOOL.FALSE;//geçerli move seçildikten sonra tekrar değiştirilmemesi için kontrol flag'ı 

var TotalSeconds = 0;
var TotalSecondsComp = 0;//Bilgisayarın sayacı için kullanılan değişken
var Increments = 0;
var TimerOn = BOOL.TRUE;
var TimerOnComp = BOOL.FALSE;//Bilgisayarın sayacının aktif olup olmaması için tutulan flag
var timing = 0;
var increment = 0;
var setClearTime;